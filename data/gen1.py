...
class VOCDetection(data.Dataset):
    ...
    def __init__(self,
        ...
        self._imgpath = osp.join(self.root , f'gen1data/{image_sets}_3ch', '%s')

        self.ids = list()

        rootpath = osp.join(self.root , 'gen1data/', f'{image_sets}.txt')
        for line in open(rootpath):
            # 00555_person_badminton_outdoor7_0.jpg 187 136 226 247 6
            parts = line.split()
            self.ids.append((parts))
            # ids如 ： self.ids [(00555_person_badminton_outdoor7_0.jpg,187,136,226,247,6)]
    ...
    def load_img_targets(self, img_id):
        # load an image
        # ids如 ： self.ids [(00555_person_badminton_outdoor7_0.jpg,187,136,226,247,6)]
        #print(osp.join(self.root,self._imgpath % img_id[0]))
        img = cv2.imread(osp.join(self._imgpath % img_id[0]))
        # self._imgpath = osp.join('testallimg', '%s')
        # s正好被id中的图像名称的变量填充
        ...
        # print(splited[0])
        num_boxes = (len(img_id) - 1) // 5
        target = []

        for i in range(num_boxes):
            x = float(img_id[1 + 5 * i])/width
            y = float(img_id[2 + 5 * i])/height
            x2 = float(img_id[3 + 5 * i])/width
            y2 = float(img_id[4 + 5 * i])/height
            c = img_id[5 + 5 * i]
            target.append([x, y, x2, y2, int(c) ])
            # target [[0.524, 0.56, 0.646, 0.9013333333333333, 8]]
    ...
    def pull_item(self, index):
        ...
        # load an image and target
            ...
            img, boxes, labels, scale, offset = self.transform(img, target[:, :4], target[:, 4])
            #t1 = time.time()
            #print('transform',t1-t0)
            
            
            #print('gtboxes',img_id)
    ...
    def pull_item_forcoco(self, index):
        # load a mosaic image
        if self.mosaic and np.random.randint(2):
            # mosaic
            img, target, height, width = self.load_mosaic(index)
        ...
        # load an image and target
            ...
            #print('gtboxes',img_id[1:5])

        target = np.hstack((boxes, np.expand_dims(labels, axis=1)))

        return img, target, height, width, scale, offset , img_id
    ...
    def pull_image(self, index):
        ...
        return cv2.imread(self._imgpath % img_id[0], cv2.IMREAD_COLOR), img_id[0]
