...
class FC(nn.Module):
    ...
    def forward(self, x):
        ...
        c_spikes = torch.zeros_like(c).detach()
        output_H = self.c2 
        #print('c_spikes.shape',c_spikes.shape)
        c_out = torch.zeros((batch_size,output_H), dtype=torch.float32,device=device)
        for step in range(self.K):
            # 
            c, c_spike = fs_coding(c, self.D[step], self.D[step])
            c_spikes += c_spike.detach()
            c_out = fs_mem_update(self.fc, c_spike, c_out, self.D[step])
...
class DenseNet(nn.Module):
    ...
    def forward(self, x, targets=None):
        if not self.trainable:
            ...
            cout,c_spikes,c_spike_n =  self.cv1(c)
            #print('cout.shape',cout.shape)
            cout =  self.bn1(cout)
            cout =  self.relu1(cout)
            cout =  self.maxpool(cout)
            
            cout,c_spikes1,c_spike_n1 = self.layer1(cout)
            cout,c_spikes2,c_spike_n2 = self.transition1(cout)
            
            cout,c_spikes3,c_spike_n3 = self.layer2(cout)
            #print('c3',cout.shape)
            cout,c_spikes4,c_spike_n4 = self.transition2(cout)
            
            
            cout,c_spikes5,c_spike_n5 = self.layer3(cout)
            #print('c4',cout.shape)
            cout,c_spikes5,c_spike_n5 = self.transition3(cout)
            
            cout,c_spikes6,c_spike_n6 = self.layer4(cout)
            #print('c5',cout.shape)
            ...
            cout,c_spikes7,c_spike_n7=self.fc1(cout)
            cout=self.bn2(cout)
            cout,c_spikes8,c_spike_n8=self.fc2(cout)
            #sys.exit
            
            c_spikes = c_spikes + c_spikes1 + c_spikes2 + c_spikes3+ c_spikes4+ c_spikes5+ c_spikes6+ c_spikes7+ c_spikes8
            c_spike_n = c_spike_n +c_spike_n1 + c_spike_n2 + c_spike_n3+ c_spike_n4+ c_spike_n5+ c_spike_n6+ c_spike_n7+ c_spike_n8 
            sc =  c_spikes/ c_spike_n
            sc = sc.item()     
...
def main():  
    ...
    path_checkpoint = './weight/densenet121_16.pth'
    checkpoint = torch.load(path_checkpoint)  
#    if 'cv1.conv.weight' in checkpoint['net']:
#        del checkpoint['net']['cv1.conv.weight']  
#        del checkpoint['net']['cv1.bn.weight']  
#        del checkpoint['net']['cv1.bn.bias']  
#        del checkpoint['net']['cv1.bn.running_mean']  
#        del checkpoint['net']['cv1.bn.running_var']  
    cnn.load_state_dict(checkpoint['net'], strict=False)
...
#                                weight_decay=5e-4)
    ...
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs*len(train_dataset)/batch_size)    
