...
def ciou_score(bboxes_a, bboxes_b, batch_size):
    ...
    w1 = bboxes_a[..., 2] - bboxes_a[..., 0]+ 1e-7
    h1 = bboxes_a[..., 3] - bboxes_a[..., 1]+ 1e-7
    w2 = bboxes_b[..., 2] - bboxes_b[..., 0]+ 1e-7
    h2 = bboxes_b[..., 3] - bboxes_b[..., 1]+ 1e-7
    v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)
    with torch.no_grad():
        alpha = v / (v - iou + (1. + 1e-14))

    ciou = iou - (rho2 / c2 + v * alpha)

    return ciou.view(batch_size, -1)

def diou_score(bboxes_a, bboxes_b, batch_size):
    """
        Input:\n
        bboxes_a : [B*N, 4] = [x1, y1, x2, y2] \n
        bboxes_b : [B*N, 4] = [x1, y1, x2, y2] \n
...
if __name__ == '__main__':
    ...
    print(ciou)
    diou = ciou_score(bboxes_a, bboxes_b, 1)
    print(diou)
