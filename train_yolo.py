...
from data.gen1 import VOCDetection
...
def parse_args():
    ...
    parser.add_argument('--max_epoch', type=int, default=100,
                        help='max_epoch')
    parser.add_argument('--lr_epoch', nargs='+', default=[0, 100], type=int,
                        help='lr epoch to decay')
    parser.add_argument('--wp_epoch', type=int, default=2,
                        help='The upper bound of warm-up')
    parser.add_argument('--start_epoch', type=int, default=0,
                        help='start epoch to train')
    parser.add_argument('-r', '--resume', default='./weight/yolo_tiny.pth', type=str,
                        help='.pth')
    ...
    parser.add_argument('--save_folder', default='weight', type=str,
    ...
    # model
    parser.add_argument('-m', '--model', default='yolo_tiny_DLN',
                        help='yolo_tiny_DLN (DL-Net), yolo_tiny_SNN (FSN-SNN)')
    ...
    parser.add_argument('--loss_reg_weight', default=2.0, type=float,
                        help='weight of reg loss')
    parser.add_argument('--scale_loss', default='batch', type=str,
                        help='scale loss: batch or positive samples')

    # train trick
    parser.add_argument('--no_warmup', action='store_true', default=True,
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            if iter_i % 100 == 0:
                ...
                t1 = time.time()
                print('[Epoch %d/%d][Iter %d/%d][lr %.6f][Loss: total %.2f || obj %.2f || cls %.2f || reg %.2f || size %d || time: %.2f]'
                        % (epoch+1, 
                           args.max_epoch, 
                           iter_i, 
                           epoch_size, 
        ...
        print('this epoch cost time',t_epoch1-t_epoch0)
        
        torch.save(model.state_dict(), os.path.join(path_to_save, args.model + '_' + repr(epoch + 1) + '.pth'))
        ...
        if (epoch + 1) % args.eval_epoch == 0 or (epoch + 1) == args.max_epoch:
            ...
            if evaluator is None:
                ...
                torch.save(model_eval.state_dict(), os.path.join(path_to_save, args.model + '_' + repr(epoch + 1) + '.pth'))
                print('Keep training ...')
            else:
                print('eval ...')
                ...
                if local_rank == 0:
                    ...
                    if cur_map > best_map:
                        ...
                        # save model
                        print('Saving state, epoch:', epoch + 1)
                        torch.save(model_eval.state_dict(), os.path.join(path_to_save, args.model + '_' + repr(epoch + 1) + '_' + str(round(best_map*100, 2)) + '.pth'))
                    if args.tfboard:
                        if args.dataset == 'voc':
        ...
        if args.mosaic and args.max_epoch - epoch == 5:
