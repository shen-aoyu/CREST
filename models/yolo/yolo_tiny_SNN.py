...
class ActFun(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return torch.where(input >= 0, torch.ones_like(input),0)

    @staticmethod
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone()
        temp = (abs(input) < lens)*(0.5/lens)
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        # output_H = (c_spikes.shape[3] + 2 * self.p - self.k) // self.s + 1
        # c_out = torch.zeros((batch_size, self.c2, output_H, output_H), dtype=torch.float32,device=device)

        a = self.conv(x)
        c_out = torch.zeros_like(a)

        for step in range(self.K):
            c, c_spike = fs_coding(c, self.D[step], self.D[step])
            
            c_spikes += c_spike
            c_out = fs_mem_update(self.conv, c_spike, c_out, self.D[step])
        c_out  = self.bn(c_out)
...
class yolo_tiny_SNN(nn.Module):
    def __init__(self, 
                 ...
                 nms_thresh=0.60,
                 center_sample=False):
        super(yolo_tiny_SNN, self).__init__()
        self.cfg = cfg
        self.device = device
    ...
    def inference_single_image(self, x):
        ...
        #sys.exit
        sc1 = torch.sum(c_spikes1) / (c_spikes1.numel())
        sc2 = torch.sum(c_spikes2) / (c_spikes2.numel())
        sc3 = torch.sum(c_spikes3) / (c_spikes3.numel())
        sc4 = torch.sum(c_spikes4) / (c_spikes4.numel())
        sc5 = torch.sum(c_spikes5) / (c_spikes5.numel())
        sc6 = torch.sum(c_spikes6) / (c_spikes6.numel())
        sc7 = torch.sum(c_spikes7) / (c_spikes7.numel())
        sc8 = torch.sum(c_spikes8) / (c_spikes8.numel())
        sc9 = torch.sum(c_spikes9) / (c_spikes9.numel())
        sc10 = torch.sum(c_spikes10) / (c_spikes10.numel())
        sc11 = torch.sum(c_spikes11) / (c_spikes11.numel())
        sc12 = torch.sum(c_spikes12) / (c_spikes12.numel())
        sc13 = torch.sum(c_spikes13) / (c_spikes13.numel())
        sc14 = torch.sum(c_spikes14) / (c_spikes14.numel())
        sc15 = torch.sum(c_spikes15) / (c_spikes15.numel())
        sc16 = torch.sum(c_spikes16) / (c_spikes16.numel())
        sc17 = torch.sum(c_spikes17) / (c_spikes17.numel())
        sc18 = torch.sum(c_spikes18) / (c_spikes18.numel())
        sc19 = torch.sum(c_spikes19) / (c_spikes19.numel())
        sc20 = torch.sum(c_spikes20) / (c_spikes20.numel())       
        sc21 = torch.sum(c_spikes21) / (c_spikes21.numel())
        sc22 = torch.sum(c_spikes22) / (c_spikes22.numel())
        sc23 = torch.sum(c_spikes23) / (c_spikes23.numel())
        sc24 = torch.sum(c_spikes24) / (c_spikes24.numel())
        sc25 = torch.sum(c_spikes25) / (c_spikes25.numel())
        sc26 = torch.sum(c_spikes26) / (c_spikes26.numel())
        sc27 = torch.sum(c_spikes27) / (c_spikes27.numel())
        sc28 = torch.sum(c_spikes28) / (c_spikes28.numel())
        sc29 = torch.sum(c_spikes29) / (c_spikes29.numel())
        sc30 = torch.sum(c_spikes30) / (c_spikes30.numel())  
        sc31 = torch.sum(c_spikes31) / (c_spikes31.numel())
        sc32 = torch.sum(c_spikes32) / (c_spikes32.numel())
        sc33 = torch.sum(c_spikes33) / (c_spikes33.numel())
        sc34 = torch.sum(c_spikes34) / (c_spikes34.numel())
        sc35 = torch.sum(c_spikes35) / (c_spikes35.numel())
        sc36 = torch.sum(c_spikes36) / (c_spikes36.numel())
        sc37 = torch.sum(c_spikes37) / (c_spikes37.numel())
        sc38 = torch.sum(c_spikes38) / (c_spikes38.numel())
        sc39 = torch.sum(c_spikes39) / (c_spikes39.numel())
        sc40 = torch.sum(c_spikes40) / (c_spikes40.numel())
        sc41 = torch.sum(c_spikes41) / (c_spikes41.numel())
        sc42 = torch.sum(c_spikes42) / (c_spikes42.numel())
        sc43 = torch.sum(c_spikes43) / (c_spikes43.numel())
        sc44 = torch.sum(c_spikes44) / (c_spikes44.numel())
        sc45 = torch.sum(c_spikes45) / (c_spikes45.numel())
        sc46 = torch.sum(c_spikes46) / (c_spikes46.numel())
        sc47 = torch.sum(c_spikes47) / (c_spikes47.numel())
        sc48 = torch.sum(c_spikes48) / (c_spikes48.numel())
        sc49 = torch.sum(c_spikes49) / (c_spikes49.numel())
        sc50 = torch.sum(c_spikes50) / (c_spikes50.numel())
        sc51 = torch.sum(c_spikes51) / (c_spikes51.numel())
        sc52 = torch.sum(c_spikes52) / (c_spikes52.numel())
        sc53 = torch.sum(c_spikes53) / (c_spikes53.numel())
        sc54 = torch.sum(c_spikes54) / (c_spikes54.numel())
        sc55 = torch.sum(c_spikes55) / (c_spikes55.numel())
        sc56 = torch.sum(c_spikes56) / (c_spikes56.numel())
        sc57 = torch.sum(c_spikes57) / (c_spikes57.numel())
        sc58 = torch.sum(c_spikes58) / (c_spikes58.numel())
        sc59 = torch.sum(c_spikes59) / (c_spikes59.numel())
        sc60 = torch.sum(c_spikes60) / (c_spikes60.numel())     
        sc61 = torch.sum(c_spikes61) / (c_spikes61.numel())    
        sc62 = torch.sum(c_spikes62) / (c_spikes62.numel())
        sc63 = torch.sum(c_spikes63) / (c_spikes63.numel())
        sc64 = torch.sum(c_spikes64) / (c_spikes64.numel())     
        
        s_ave = (torch.sum(c_spikes1) + torch.sum(c_spikes2) + torch.sum(c_spikes3)+torch.sum(c_spikes4) + torch.sum(c_spikes5) + 
                 torch.sum(c_spikes6) + torch.sum(c_spikes7) + torch.sum(c_spikes8) + torch.sum(c_spikes9) + torch.sum(c_spikes10) +
                 torch.sum(c_spikes11) + torch.sum(c_spikes12) + torch.sum(c_spikes13)+torch.sum(c_spikes14) + torch.sum(c_spikes15) + 
                 torch.sum(c_spikes16) + torch.sum(c_spikes17) + torch.sum(c_spikes18) + torch.sum(c_spikes19) + torch.sum(c_spikes20) +
                 torch.sum(c_spikes21) + torch.sum(c_spikes22) + torch.sum(c_spikes23)+torch.sum(c_spikes24) + torch.sum(c_spikes25) + 
                 torch.sum(c_spikes26) + torch.sum(c_spikes27) + torch.sum(c_spikes28) + torch.sum(c_spikes29) + torch.sum(c_spikes30) +    
                 torch.sum(c_spikes31) + torch.sum(c_spikes32) + torch.sum(c_spikes33)+torch.sum(c_spikes34) + torch.sum(c_spikes35) + 
                 torch.sum(c_spikes36) + torch.sum(c_spikes37) + torch.sum(c_spikes38) + torch.sum(c_spikes39) + torch.sum(c_spikes40) +   
                 torch.sum(c_spikes41) + torch.sum(c_spikes42) + torch.sum(c_spikes43)+torch.sum(c_spikes44) + torch.sum(c_spikes45) + 
                 torch.sum(c_spikes46) + torch.sum(c_spikes47) + torch.sum(c_spikes48) + torch.sum(c_spikes49) + torch.sum(c_spikes50) +  
                 torch.sum(c_spikes51) + torch.sum(c_spikes52) + torch.sum(c_spikes53)+torch.sum(c_spikes54) + torch.sum(c_spikes55) + 
                 torch.sum(c_spikes56) + torch.sum(c_spikes57) + torch.sum(c_spikes58) + torch.sum(c_spikes59) + torch.sum(c_spikes60) +                                                 
                 torch.sum(c_spikes61) + torch.sum(c_spikes62) + torch.sum(c_spikes63)+ torch.sum(c_spikes64)) / (c_spikes1.numel() + c_spikes2.numel() + c_spikes3.numel() + 
                 c_spikes4.numel() + c_spikes5.numel() +
                 c_spikes6.numel() + c_spikes7.numel() + c_spikes8.numel() + c_spikes9.numel() + c_spikes10.numel() +
                 c_spikes11.numel() + c_spikes12.numel() + c_spikes13.numel() + c_spikes14.numel() + c_spikes15.numel() +
                 c_spikes16.numel() + c_spikes17.numel() + c_spikes18.numel() + c_spikes19.numel() + c_spikes20.numel() +
                 c_spikes21.numel() + c_spikes22.numel() + c_spikes23.numel() + c_spikes24.numel() + c_spikes25.numel() +
                 c_spikes26.numel() + c_spikes27.numel() + c_spikes28.numel() + c_spikes29.numel() + c_spikes30.numel() +
                 c_spikes31.numel() + c_spikes32.numel() + c_spikes33.numel() + c_spikes34.numel() + c_spikes35.numel() +
                 c_spikes36.numel() + c_spikes37.numel() + c_spikes38.numel() + c_spikes39.numel() + c_spikes40.numel() +
                 c_spikes41.numel() + c_spikes42.numel() + c_spikes43.numel() + c_spikes44.numel() + c_spikes45.numel() +
                 c_spikes46.numel() + c_spikes47.numel() + c_spikes48.numel() + c_spikes49.numel() + c_spikes50.numel() +
                 c_spikes51.numel() + c_spikes52.numel() + c_spikes53.numel() + c_spikes54.numel() + c_spikes55.numel() +
                 c_spikes56.numel() + c_spikes57.numel() + c_spikes58.numel() + c_spikes59.numel() + c_spikes60.numel() +
                 c_spikes61.numel() + c_spikes62.numel() + c_spikes63.numel() + c_spikes64.numel() )        
        

        sc = [sc1.item(), sc2.item(), sc3.item(), sc4.item(), sc5.item(), sc6.item(), sc7.item(), sc8.item(), sc9.item(), sc10.item() ,sc11.item(), sc12.item(), sc13.item(), sc14.item(), sc15.item(), sc16.item(), sc17.item(), sc18.item(), sc19.item(), sc20.item() , sc21.item(), sc22.item(), sc23.item(), sc24.item(), sc25.item(), sc26.item(), sc27.item(), sc28.item(), sc29.item(), sc30.item() ,sc31.item(), sc32.item(), sc33.item(), sc34.item(), sc35.item(), sc36.item(), sc37.item(), sc38.item(), sc39.item(), sc40.item() ,sc41.item(), sc42.item(), sc43.item(), sc44.item(), sc45.item(), sc46.item(), sc47.item(), sc48.item(), sc49.item(), sc50.item() ,sc51.item(), sc52.item(), sc53.item(), sc54.item(), sc55.item(), sc56.item(), sc57.item(), sc58.item(), sc59.item(), sc60.item() ,sc61.item(), sc62.item(), sc63.item(), sc64.item(),s_ave.item()]
        sc = ( np.array(sc)[-1] )/ K[1]
        ...
        return bboxes, scores, cls_inds,sc
        #return 0, 0, 0,sc
    ...
    def forward(self, x, targets=None):
        if not self.trainable:
            ...
            iou_target = 1*densitys_iou_target
