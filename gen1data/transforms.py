...
class Resize(object):
    ...
    def __call__(self, image, boxes=None, labels=None, scale=None, offset=None):
        ...
        if h0 > w0:
            ...
            # zero padding
            h, w, _ = image.shape
            image_ = np.ones([h, h, 5]) * self.mean
            dw = h - w
            left = dw // 2
            ...
            # zero padding
            h, w, _ = image.shape
            image_ = np.ones([w, w, 5]) * self.mean
            dh = w - h
            top = dh // 2
...
class ResizeGrayscale(object):  
    def __init__(self, size=640, mean=None):  
        self.size = size  
        # 注意：对于单通道图像，mean通常是一个单一的浮点数值，用于填充  
    ...
    def __call__(self, image, boxes=None, labels=None, scale=None, offset=None):  
        ...
        if isinstance(image.shape, torch.Size):
...
#            pass
#        else:  
#            image = torch.from_numpy(image.astype(np.float32))  # 添加一个通道维度  
#        return image, boxes, labels, scale, offset

class grayToTensor(object):  
    def __call__(self, image, boxes=None, labels=None, scale=None, offset=None):  
        # 确保图像是浮点数，并且范围在[0, 1]之间（如果尚未归一化）  
        # 对于单通道图像，不需要进行通道重排  
        if isinstance(image, torch.Tensor):
            pass
        else:  
            image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)  # 添加一个通道维度  
            
        
          
        # PyTorch期望图像张量的形状为(C, H, W)，其中C是通道数  
        # 因此，我们需要将单通道图像的形状从(H, W)变为(1, H, W)  
        #image = image.permute(2, 0, 1)  # 假设原始NumPy数组的形状是(H, W)  
        #print('image.shape',image.shape)
        # 返回转换后的图像和其他参数（如果有）  
        return image, boxes, labels, scale, offset  
...
class ValTransforms(object):
    def __init__(self, size=640, mean=(0), std=(0)):
        self.size = size
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.augment = Compose([
...
#        ResizeGrayscale(self.size, self.mean),
#        grayToTensor()
# grayToTensor()
